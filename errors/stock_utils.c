...
void    resolution()
{
   ...
   // int     i;
    ...
    nu_information(line) != 3 ? ft_print_errors(4) : 3;
    ft_strlen(line[0]) != 1 ? ft_print_errors(1) : 1;
    g_ray.win_w = ft_atoi(line[1]);
    g_ray.win_w == 0 ? ft_print_errors(2) : g_ray.win_w;
    g_ray.win_w = g_ray.win_w > WIN_WMAX ? WIN_WMAX : g_ray.win_w;
   // printf("win_W: %d\n", g_ray.win_w);
    g_ray.win_h = ft_atoi(line[2]);
    g_ray.win_h == 0 ? ft_print_errors(3) : g_ray.win_h;
