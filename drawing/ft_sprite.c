...
void    ft_get_data_sprite(void)
{
    ...
    while (index < g_p.count_spri)
    {
        ...
        if ((g_dt.ro_angle - g_dt.fov_angle / 2) - g_sprite[index].ang > M_PI)
			g_sprite[index].ang += 2 * M_PI;
		if (g_sprite[index].ang - (g_dt.ro_angle - g_dt.fov_angle / 2) > M_PI)
			g_sprite[index].ang -= 2 * M_PI;
        index++;
    }
}
...
void	ft_render_sprite(int x, double distance, double stripe_h, int bottom)
{
	...
	i = x;
    bottom = (g_dt.win_h + stripe_h) / 2;
	while (i < (x + stripe_h))
	{
		top = (g_dt.win_h - stripe_h) / 2;
		y = 0;
		if (i >= 0 && i < g_dt.win_w && distance < g_r1[i].dis_t)
		{
			while (top < bottom)
			{
                ...
                {
                    color_spr = g_tex.addr[4][color_spr];
                    if (top < g_dt.win_h && top >= 0 && color_spr > 0x000000)
                        g_dt.addr[(top * g_dt.win_w) + i] = color_spr;
                }
                top++;
				y++;
			}
...
void	ft_draw_sprite(void)
{
	...
	while (i < g_p.count_spri)
	{
		...
		column_c = (g_sprite[i].ang - (g_dt.ro_angle  - g_dt.fov_angle / 2)) / (g_dt.fov_angle / g_dt.win_w);
		ft_render_sprite(column_c - (stripe_h / 2), g_sprite[i].dis, stripe_h, 0);
		i++;
	}
}